...
sys.path.append('../scale_invariant/')
import zipfile
import re
import sys
import os
...
def validate_lines_in_file_gt(fileName,file_contents,CRLF=True,LTRB=True,withTranscription=False,withConfidence=False,imWidth=0,imHeight=0, line_format='totaltext'):
    """
    ...
    for line in lines:
        ...
        if(line != ""):
            try:
                validate_tl_line_gt(line,LTRB,withTranscription,withConfidence,imWidth,imHeight, line_format)
...
def validate_tl_line_gt(line,LTRB=True,withTranscription=True,withConfidence=True,imWidth=0,imHeight=0, line_format='totaltext'):
    """
    ...
    """
    get_tl_line_values_gt(line,LTRB,withTranscription,withConfidence,imWidth,imHeight, line_format)
...
def get_tl_line_values_gt(line,LTRB=True,withTranscription=False,withConfidence=False,imWidth=0,imHeight=0, line_format='totaltext'):
    """
    Validate the format of the line. If the line is not valid an exception will be raised.
    If maxWidth and maxHeight are specified, all points must be inside the imgage bounds.
    Posible values are:
    ...
    if LTRB:
        ...
        if withTranscription and withConfidence:
            ...
            if line_format == 'totaltext':
                ptr = line.strip().split(',####')
                cors = ptr[0].split(',')
                recs = str(ptr[1].strip())
            elif line_format.startswith('icdar'):
                numPoints=8
                m = re.match(
                ...
                cors = [float(m.group(i)) for i in range(1, (numPoints + 1))]
                posTranscription = numPoints + (2 if withConfidence else 1)
                recs = m.group(posTranscription)
            assert(len(cors)%2 == 0), 'num cors should be even.'
            try:
                points = [ float(ic) for ic in cors[:]]
            except Exception as e:
...
def get_tl_line_values_from_file_contents(content,CRLF=True,LTRB=True,withTranscription=False,withConfidence=False,imWidth=0,imHeight=0,sort_by_confidences=True, line_format='totaltext'):
    """
    ...
    for line in lines:
        line = line.replace("\r","").replace("\n","")
        if(line != "") :
            points, confidence, transcription = get_tl_line_values_gt(line,LTRB,withTranscription,withConfidence,imWidth,imHeight,line_format);
