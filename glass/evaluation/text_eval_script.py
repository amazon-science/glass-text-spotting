...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    """
    line_format= 'totaltext' if 'totaltext' in gtFilePath or 'textocr' in gtFilePath else 'icdar'
    if line_format.startswith('icdar'):
        evaluationParams['GT_SAMPLE_NAME_2_ID'] = 'gt_img_([0-9]+).txt'  # for icdar15
    ...
    for k in gt:
        rrc_evaluation_funcs.validate_lines_in_file_gt(k, gt[k], evaluationParams['CRLF'], evaluationParams['LTRB'], True,  line_format=line_format)
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    for resFile in gt:
        ...
        pointsList,_,transcriptionsList = rrc_evaluation_funcs.get_tl_line_values_from_file_contents(gtFile, evaluationParams['CRLF'], evaluationParams['LTRB'], True, False, line_format=line_format)
                    ...
                    for detNum in range(len(detPols)):
                        if gtRectMat[gtNum] == 0 and detRectMat[detNum] == 0 and gtNum not in gtDontCarePolsNum and detNum not in detDontCarePolsNum :
                            if iouMat[gtNum,detNum]>evaluationParams['IOU_CONSTRAINT']:
                                ...
                                # detOnlyCorrect += 1
                                gt_word = gtTrans[gtNum].upper().replace('####', '')
                                pred_word = detTrans[detNum].upper()
                                if evaluationParams['WORD_SPOTTING']:
                                    edd = lstn.distance(gt_word, pred_word)
                                    if edd<=0: 
                                        correct = True
                                    else:
                                        correct = False
                                    ...
                                    try:
                                        correct = transcription_match(gt_word,pred_word,evaluationParams['SPECIAL_CHARACTERS'],evaluationParams['ONLY_REMOVE_FIRST_LAST_CHARACTER'])==True
                                    except: # empty
                                        correct = False
